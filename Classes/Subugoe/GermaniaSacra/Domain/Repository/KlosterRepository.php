...
class KlosterRepository extends Repository
{
    ...
    */
    protected $entities = ['kloster_id' => 'kloster', 'bearbeitungsstatus' => 'bearbeitungsstatus', 'kloster' => 'kloster', 'ort' => 'ort', 'gnd' => 'url', 'bearbeitungsstand' => 'kloster'];

    /**
    * @var array An array of associated entities for ordering
    */
    protected $orderByEntities = ['kloster_id' => 'kloster', 'name' => 'bearbeitungsstatus', 'kloster' => 'kloster', 'ort' => 'ort', 'url' => 'url', 'bearbeitungsstand' => 'kloster'];
    ...
    public function findLastEntry($offset=0, $limit=1)
    {
        ...
        $query->matching($query->lessThan('kloster_id', 20000))
                ->setOrderings(['kloster_id' => \TYPO3\Flow\Persistence\QueryInterface::ORDER_DESCENDING])
                ->setOffset($offset)
                ->setLimit($limit);
        return $query->execute();
    }
    ...
    /** @var $queryBuilder \Doctrine\ORM\QueryBuilder **/
        ...
        if (is_array($searchArr) && count($searchArr) > 0) {
            ...
            foreach ($searchArr as $k => $v) {
                ...
                $filter = $v['filter'];
                if ($filter === 'ort.ort') {
                    $ortFilterExists = true;
                }
                if ($filter === 'bearbeitungsstatus.name') {
                    $bearbeitungsstatusFilterExists = true;
                }
